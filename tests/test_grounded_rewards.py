...
class TestGroundedFormatReward:
    ...
    def test_missing_key_partial_score(self):
        ...
        assert rewards == [0.5]  # 0.25 (parseable) + 0.25 (partial keys still present? no)
        # Actually: 0.25 (JSON ok) + 0.0 (missing key) = 0.25
        # Let's check: REQUIRED_KEYS not subset → return after 0.5? No, after 0.25 + the keys check.
        # REQUIRED_KEYS = {reasoning_path, is_context_sufficient, final_answer, extracted_quotes}
        # incomplete is missing extracted_quotes → REQUIRED_KEYS.issubset fails → return 0.25
        assert rewards == [0.25]
